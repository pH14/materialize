...
class EnvironmentdStatefulSet(K8sStatefulSet):
    ...
    def args(self) -> list[str]:
        ...
        if self._meets_minimum_version("0.79.0-dev"):
            args += [
                "--timestamp-oracle-url=postgres://postgres@postgres:postgres:5432"
            ]
        if not self._meets_minimum_version("0.105.0-dev"):
            args += [
                "--storage-stash-url=postgres://root@postgres@postgres:postgres:5432"
            ]
    ...
    def env_vars(self) -> list[V1EnvVar]:
        ...
        env = [
            ...
            V1EnvVar(
                ...
                value="postgres://postgres@postgres:postgres:5432",
            ),
        ]
